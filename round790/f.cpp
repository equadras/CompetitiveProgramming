...
void solve(){
    ...
    //ver quais elementos tem freq > x
    vector<int> aux;
    for (auto ele : mp){
        if (ele.second >= x) aux.push_back(ele.first);
    }
    
    if (aux.size() == 0){ cout << -1 << endl; return;}

    //achar qual e maior swq
     else {
        sort (aux.begin(), aux.end());
        int maxi = 0;
        int resL = aux[0], resR = aux[0], lmao = aux[0];
